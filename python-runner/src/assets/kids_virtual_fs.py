...
try:
    ...
    def _vfs_pil_open(path, *args, **kwargs):
        if isinstance(path, (str, bytes)):
            key = _normalize(path.decode() if isinstance(path, bytes) else path)
            if key in VFS:
                return _orig_pil_open(BytesIO(VFS[key]), *args, **kwargs)
        return _orig_pil_open(path, *args, **kwargs)

    Image.open = _vfs_pil_open
except Exception:
    pass

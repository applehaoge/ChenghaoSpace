...
const requestVisionCaption = async (buffer: Buffer, mimeType: string, displayName: string): Promise<string> => {
  ...
  try {
    ...
    if (Array.isArray(content)) {
      return content.map((segment: any) => (typeof segment === 'string' ? segment : segment?.text || '')).join('').trim();
    }
...
const buildMetadataFallback = (record: StoredUploadRecord, width?: number, height?: number) => {
  const parts = [
...
export const analyzeImage = async (record: StoredUploadRecord): Promise<ImageInsight> => {
  ...
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      ...
      warnings.push(
        `图片体积为 ${formatBytes(buffer.byteLength)}，超过限制 ${formatBytes(MAX_IMAGE_BYTES)}，跳过详细识别。`
      );
      caption = buildMetadataFallback(record, width, height);
    } else {
      try {

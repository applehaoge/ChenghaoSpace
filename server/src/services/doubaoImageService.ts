...
import { analyzeImage as analyzeWithOpenAI, type ImageInsight } from './imageAnalyzer.js';

const DEFAULT_MODEL = 'doubao-seed-1-6-flash';
const DEFAULT_PROMPT =
  '请用中文详细描述这张图片（文件名：{filename}）的场景、关键元素、出现的文字信息以及可能需要注意的风险点。';
const DEFAULT_TIMEOUT = 20000;
const DEFAULT_MAX_BYTES = 8 * 1024 * 1024;

const knownMimeByExt: Record<string, string> = {
  '.jpg': 'image/jpeg',
  '.jpeg': 'image/jpeg',
  '.png': 'image/png',
  '.gif': 'image/gif',
...
const requestWithProxy = async (url: string, init: RequestInit): Promise<Response> => {
  ...
  try {
    return await fetch(url, { ...init, agent: agent as any });
  } catch (err) {
    if (shouldBypassProxy(err)) {
      console.warn(
        '[DoubaoImageService] Proxy request failed, retrying without proxy:',
...
const resolveMimeType = (record: StoredUploadRecord) => {
...
export const analyzeImageWithDoubao = async (
  ...
  if (Number.isFinite(maxBytes) && maxBytes > 0 && buffer.byteLength > maxBytes) {
    throw new Error(
      `图片体积 ${Math.round(buffer.byteLength / 1024)}KB 超出限制（${Math.round(maxBytes / 1024)}KB），已跳过豆包识别`
    );
  }
  ...
  const model =
    (process.env.DOUBAO_IMAGE_MODEL || process.env.DOUBAO_CHAT_MODEL || '').trim() || DEFAULT_MODEL;
  ...
  if (!Number.isNaN(temperature) && temperature >= 0) parameters.temperature = temperature;

  const maxTokens = Number(maxTokensRaw);
  if (!Number.isNaN(maxTokens) && maxTokens > 0) parameters.max_output_tokens = maxTokens;

  const topP = Number(topPRaw);
  if (!Number.isNaN(topP) && topP > 0 && topP <= 1) parameters.top_p = topP;
...
export const analyzeAttachmentImage = async (
  record: StoredUploadRecord,
  options: { prompt?: string } = {}
): Promise<DoubaoImageInsight> => {
  ...
  try {
    ...
    const nextWarnings = new Set<string>(fallback.warnings);

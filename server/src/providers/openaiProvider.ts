...
export class OpenAIProvider implements LLMProvider {
  ...
  baseUrl: string;
  apiPath?: string;
  constructor(apiKey: string, baseUrl?: string, apiPath?: string) {
    this.apiKey = apiKey;
  ...
  async embed(text: string | string[]) {
    ...
    if (!data || !data.data) throw new Error('invalid embeddings response');
    const embeddings: any = (data as any).data.map((d: any) => d.embedding);

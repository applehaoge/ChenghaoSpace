...
// ===== 路径校验 =====
const ensureSafePath = (rawPath: any): string => {
...
// ===== 编码校验 =====
const ensureEncoding = (encoding: any): RunFileEncoding => {
...
const ensureContent = (rawContent: any, encoding: RunFileEncoding, path: string): string => {
...
export const sanitizeRunJobDTO = (payload: any): RunJobDTO => {
  ...
  if (payload.files.length > MAX_FILE_COUNT) {
    throwValidationError(
      'FILE_COUNT_EXCEEDED',
      `Python 文件数量超过限制（最多 ${MAX_FILE_COUNT} 个）`,
    );
  }
  ...
  for (const file of payload.files as any[]) {

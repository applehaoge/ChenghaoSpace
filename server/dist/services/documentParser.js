...
const buildExcerpt = (text, maxChars) => {
...
const readTextFile = async (record) => {
...
const readPdfFile = async (record) => {
    const buffer = await fs.readFile(record.storedPath);
    ...
    if (Number.isFinite(maxBytes) && maxBytes > 0 && buffer.byteLength > maxBytes) {
        warnings.push(`文件体积 ${Math.round(buffer.byteLength / 1024)}KB 超过解析上限 (${Math.round(maxBytes / 1024)}KB)，仅尝试截取前半部分。`);
    }
...
const readDocxFile = async (record) => {

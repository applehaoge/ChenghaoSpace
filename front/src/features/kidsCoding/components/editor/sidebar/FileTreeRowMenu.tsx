...
const MENU_ITEMS: MenuItemConfig[] = [
  { action: 'rename', label: '重命名', icon: <Edit3 size={14} /> },
  { action: 'duplicate', label: '创建副本', icon: <Files size={14} /> },
  { action: 'delete', label: '删除', icon: <Trash2 size={14} />, tone: 'danger' },
  { action: 'copyName', label: '复制名称', icon: <Copy size={14} /> },
  { action: 'copyRelativePath', label: '复制相对路径', icon: <Link2 size={14} /> },
  { action: 'export', label: '导出', icon: <Download size={14} /> },
];
...
export function FileTreeRowMenu({ isDark, onSelect, menuRef }: FileTreeRowMenuProps) {
  return (
    <motion.div
      ...
      className={clsx(
        'min-w-[168px] rounded-2xl border px-1.5 py-1 shadow-xl backdrop-blur-sm',
        ...
        {MENU_ITEMS.map(item => (
          <li key={item.action}>
            <button
              ...
              className={clsx(
                'flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2',
                isDark
